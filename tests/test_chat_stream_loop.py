...
async def test_pricing_then_prepare_order_then_streamed_answer():
    ...
    tc_order = [{"function": {"name": "prepare_order", "arguments": {"items": [{"product_id": 12, "variant_id": 45, "quantity": 2}]}}}]
    draft = json.dumps({"items": [{"product_id": "12", "variant_id": "45", "quantity": 2}], "total": 6400000.0, "currency": "VND", "warnings": []})
    llm = _FakeLLM([([], tc_price), ([], tc_order), (["Mời bạn ", "bấm XÁC NHẬN."], None)])
    disp = _Dispatcher({"get_pricing": json.dumps({"productId": 12, "variants": []}), "prepare_order": draft})
